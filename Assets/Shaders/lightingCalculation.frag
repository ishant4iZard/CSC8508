...
vec3 SpotLight(vec3 inPos, vec3 inDir, float inCutoffAngle, vec3 inColor, vec3 normal, float roughness, float metallic, vec3 albedo)
{
    ...
    vec3 tempLightDir = normalize(inPos - texture(fragmentPosition, IN.texCoord).rgb); //Direction vector from fragment to spot light
    ...
    vec3 radiance = inColor * CalculateSpotLightAttenuation(inPos, tempSpotDir, tempLightDir, texture(fragmentPosition, IN.texCoord).rgb, 1.0f, 0.22f, 0.20f);
...
void main()
{
    ...
    lOut += DirectionalLightPbr(globalLightDirection, texture(fragmentPosition, IN.texCoord).rgb, globalLightColor.rgb, normal, roughness, metallic, albedo, cameraPos);
